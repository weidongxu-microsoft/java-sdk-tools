...
server.registerTool(
  ...
  {
    description:
      "Build the Java SDK. The SDK is generated from TypeSpec source files.",
    inputSchema: {
      moduleDirectory: z
        .string()
        .describe(
          "The absolute path to the Java module. The folder name should be `java-sdk`.",
        ),
    },
    annotations: {
  ...
  async (args) => {
    logToolCall("buildJavaSdk");
    return await buildJavaSdk(args.moduleDirectory);
  },
);

// Tool: getJavaSdkChangelog
server.registerTool(
  "getJavaSdkChangelog",
  {
    description:
    ...
    inputSchema: {
      ...
      groupId: z
        .string()
        .describe("The Maven groupId for the Java SDK module."),
      artifactId: z
        .string()
        .describe("The Maven artifactId for the Java SDK module."),
    },
    annotations: {
  ...
  async (args) => {
    logToolCall("getJavaSdkChangelog");
...
server.registerTool(
  ...
  async (args) => {
    logToolCall("generateJavaSdk");
    const result = await generateJavaSdk(args.typespecSourceDirectory);
    return result;
  },
);
